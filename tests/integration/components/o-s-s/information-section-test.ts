...
module('Integration | Component | o-s-s/information-section', function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.icon = 'fa-pen';
    this.svgIcon = '/assets/images/upfluence-white-logo.svg';
    this.actionButton = {
      label: 'Test Button',
      icon: 'fa-pen'
    };
  ...
  module('badge icons', function () {
    ...
    test('it renders the icon with an svg icon as @badgeIcon arg', async function (assert) {
      await render(hbs`<OSS::InformationSection @svgIcon={{this.svgIcon}} @title={{this.title}}/>`);
      assert.dom('.upf-badge img').hasAttribute('src', this.svgIcon);
    });
